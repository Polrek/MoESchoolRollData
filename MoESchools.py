...
def load_roll_data(roll_file: str, selected_year: int, logger: logging.Logger) -> pd.DataFrame:
    ...
    try:
        ...
        # Only accept .csv files for roll data
        _, ext = os.path.splitext(roll_file)
        ext = ext.lower()
        if ext != '.csv':
            raise RuntimeError("Roll data must be a .csv file. Please provide the official MoE CSV file.")

        # Read CSV
        df = pd.read_csv(roll_file)
        logger.debug(f"Loaded CSV '{roll_file}' with shape {df.shape}")
        
        # The CSV layout uses positional columns (1-indexed in your description):
        # col1 = roll count, col2 = year, col3 = year level (e.g. 'Year 1'),
        # col5 = school_id, col6 = ethnicity (ignored). We aggregate across ethnicity.
        if df.shape[1] < 6:
            raise RuntimeError(f"Expected at least 6 columns in roll CSV, found {df.shape[1]}")

        count_col = df.columns[0]
        year_col = df.columns[1]
        yearlevel_col = df.columns[2]
        schoolid_col = df.columns[4]

        # Filter rows to the selected year (year column may be int or string)
        df_filtered = df[df[year_col].astype(str).str.strip() == str(selected_year)].copy()
        logger.debug(f"Rows for year {selected_year}: {len(df_filtered)} / {len(df)}")

        # Convert counts and school id to numeric
        df_filtered[count_col] = pd.to_numeric(df_filtered[count_col], errors='coerce').fillna(0).astype(int)
        df_filtered[schoolid_col] = pd.to_numeric(df_filtered[schoolid_col], errors='coerce')
        df_filtered = df_filtered.dropna(subset=[schoolid_col])
        df_filtered[schoolid_col] = df_filtered[schoolid_col].astype(int)

        # Extract year number from yearlevel_col and group by school and yearlevel
        def extract_yearnum(s):
            s = str(s)
            m = re.search(r"(?i)(?:year|yr|y)\s*[:._\-]*\s*(1[0-5]|[1-9])\b", s)
            if not m:
                m = re.search(r"(?<!\d)(1[0-5]|[1-9])(?!\d)", s)
            return int(m.group(1)) if m else None

        df_filtered['_YearNum'] = df_filtered[yearlevel_col].apply(extract_yearnum)
        df_filtered = df_filtered.dropna(subset=['_YearNum'])
        df_filtered['_YearNum'] = df_filtered['_YearNum'].astype(int)

        grouped = (
            df_filtered.groupby([schoolid_col, '_YearNum'])[count_col]
            .sum()
            .reset_index()
        )

        # Pivot to have Year_1..Year_15 for each school
        pivot = grouped.pivot_table(index=schoolid_col, columns='_YearNum', values=count_col, aggfunc='sum', fill_value=0)

        # Ensure columns 1..15 exist
        for y in range(1, 16):
        ...
        df_years = pivot.reset_index()
        # Rename numeric columns to Year_1..Year_15
        rename_map = {y: f'Year_{y}' for y in range(1, 16)}
        df_years = df_years.rename(columns=rename_map)

        # Normalize school id column name to 'School_Id' to match API results
        if schoolid_col != 'School_Id':
            if schoolid_col in df_years.columns:
                df_years = df_years.rename(columns={schoolid_col: 'School_Id'})
        # Ensure School_Id exists and is integer
        if 'School_Id' in df_years.columns:
            df_years['School_Id'] = pd.to_numeric(df_years['School_Id'], errors='coerce').fillna(0).astype(int)
        else:
            # Shouldn't happen, but create from pivot index if missing
            df_years['School_Id'] = df_years.index.astype(int)

        # Ensure column order: School_Id then Year_1..Year_15
        cols_order = ['School_Id'] + [f'Year_{y}' for y in range(1, 16)]
        df_years = df_years.reindex(columns=cols_order)

        logger.info(f"Loaded {len(df_years)} schools with roll data for year {selected_year}")
        return df_years
...
class App(tk.Tk):
    ...
    def _set_icon(self):
        """Set window icon from assets/image.ico; fallback to PhotoImage if needed."""
        try:
            ico_path = os.path.join(os.path.dirname(__file__), "assets", "image.ico")
            if os.path.exists(ico_path):
                self.iconbitmap(ico_path)  # best on Windows title bar/message windows
            else:
                # Fallback: try PNG if provided
                png_path = os.path.join(os.path.dirname(__file__), "assets", "image.png")
...
This field is REQUIRED to generate exports."""
    ...
    def show_info(self):
        ...
        info_text = f"""This application is designed to simplfy the MoE data load process. It accesses the publically available endpoint for Schools data, formats it and outputs a csv (optional xlsx) file for import.
...
pyinstaller --onefile --noconsole --name "MoeSchools_b{BUILD_STR}" --icon assets\\image.ico --add-data "assets\\image.ico;assets" MoESchools.py
"""
